...
export const AutocompleteTextField: FC<Props> = memo(
  ({ onBlur, onSelect, placeholder, options, disabledOptions }) => {
    ...
    const handleChange = useCallback(
      (_: unknown, v: string | null, reason: AutocompleteChangeReason) => {
        if (v === null) {
          return;
        }
        if (v.trim() === "" || disabledOptionsSet.has(v.trim())) {
          return;
        }
    ...
    const handleInputChange = useCallback((_: unknown, v: string) => {
      setInputValue(v);
    }, []);
