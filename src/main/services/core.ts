...
export const registerIpcMainServices = (providerName: string, serviceProvider: ServiceProvider) => {
  for (const [service, handler] of Object.entries(serviceProvider)) {
    ipcMain.handle(`${providerName}:${service}`, (_, ...args) => handler(...args))
  }
  ...
  if (!app.isPackaged) {
    writeFileSync(
      join(__dirname, '..', '..', 'src', 'preload', `${providerName}.gen.d.ts`),
